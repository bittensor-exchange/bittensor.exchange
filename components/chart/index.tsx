...
export default function Chart() {
    ...
    const drawChart = () => {
        ...
        const chart = createChart(ref.current, { 
            ...
            grid: {
                horzLines: {
                    color: colors[colorMode].horzLines
                },
                vertLines: {
                    color: colors[colorMode].vertLines
                },
            },
            layout: {
                textColor: colors[colorMode].textColor,
                background: {
                    type: ColorType.Solid,
                    color: colors[colorMode].backgroundColor
                }
            }
        });
    ...
    useEffect(() => {
        const onWindowResize = () => {
            const height = (Math.max(window.innerHeight, 680) - 240 - 64);
            setMaxHeight(height)
