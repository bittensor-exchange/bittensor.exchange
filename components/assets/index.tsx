    ...
    <div className="w-[250px] flex flex-col px-4 border-r dark:border-zinc-800 space-y-2">
      <div className="py-2 flex items-end pt-4 pl-2">
        ...
        <IconButton
          ...
          onClick={() => setHideAsset(!hideAsset)}
        >
          {!hideAsset ? (
      ...
      <div className="flex items-end pl-2 pt-2">
        {!hideAsset ? (
          <div>
            {numbro(totalBtc).format({ thousandSeparated: true, mantissa: 5 })}{" "}
            BTC
          </div>
        ) : (
          <div className="flex gap-2">
            <Skeleton animation={false} height={22} width={50} />
            BTC
          </div>
        )}
        {!hideAsset ? (
          <div className="pl-2 text-[14px] text-zinc-400">
            ≈ {numbro(totalBtc * quote_price).formatCurrency({mantissa: 5})}
          </div>
        ) : (
          <div className="pl-2 flex text-[14px] text-zinc-400">
            ≈ $<Skeleton animation={false} height={22} width={50} />
          </div>
        )}
      </div>
      ...
      <div>
        <div className="text-[12px] pt-8 pb-1 flex">
          <div className="w-[130px]">Coin</div>
          ...
          <div key={index} className="text-[12px] flex items-center py-1">
            ...
            <div className="w-[100px] text-right">
              <IconButton
                ...
                onClick={() => {
                  showDepositModal(true), setSelectedCoin(index);
                }}
              >
              ...
              <IconButton
                ...
                sx={{ width: 24, height: 24 }}
                onClick={() => {
                  showWithdrawModal(true), setSelectedCoin(index);
                }}
              >
