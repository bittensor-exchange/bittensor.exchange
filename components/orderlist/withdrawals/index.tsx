...
export default function Withdrawals() {
  ...
  const columns: GridColDef[] = [
    { field: 'token', flex: 0.5, headerName: 'Coin', minWidth: 50 },
    ...
    { field: 'withdrawal_address', flex: 0.5, headerName: 'Address', minWidth: 400},
    { field: 'amount', flex: 0.5, headerName: 'Amount', minWidth: 100,
      valueGetter: (params: GridValueGetterParams) =>
      `${numbro(params.row.amount).format({mantissa: 6, trimMantissa: true})}`,
    },
  ...
  const { data: rows, loading, mutate } = useApi('/api/withdraw');
